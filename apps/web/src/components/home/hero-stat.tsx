...
export function HeroStat({ eyebrow, value, meta, tone = "default" }: Props) {
  return (
    <Card className="card-hover animate-fade-in py-0 min-w-0">
      <CardContent className="p-phi-4 flex flex-col justify-between min-h-[118px] min-w-0">
        <div className="card-title">{eyebrow}</div>
        <div
          className={
            "font-display leading-[1.1] truncate mt-phi-2 " +
        ...
        <div className="text-muted-foreground text-xs mt-phi-2 truncate">{meta}</div>
      </CardContent>
    </Card>
  );
}

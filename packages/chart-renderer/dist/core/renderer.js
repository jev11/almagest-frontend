 ...
 */
export function renderRadix(options) {
    ...
    const dim = { cx, cy, radius, dpr };
    // Render layers in order (back to front)
    if (layers.background)
        drawBackground(ctx, data, theme, dim);
    if (layers.zodiacRing)
        drawZodiacRing(ctx, data, theme, dim);
    if (layers.houseOverlay)
        drawHouseOverlay(ctx, data, theme, dim);
    if (layers.aspectWeb)
        drawAspectWeb(ctx, data, theme, dim);
    if (layers.planetRing)
        drawPlanetRing(ctx, data, theme, dim);
    if (layers.degreeLabels)
        drawDegreeLabels(ctx, data, theme, dim);
    return dim;
}
/**
 ...
 */
export function renderBiwheel(options) {
    ...
    const innerDim = { cx, cy, radius: innerRadius, dpr };
    const outerDim = { cx, cy, radius: totalRadius, dpr };
 ...
 */
export function renderAspectLayer(ctx, data, theme, dim) {
    drawAspectWeb(ctx, data, theme, dim);
}

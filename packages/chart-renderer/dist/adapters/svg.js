import { SIGN_ORDER, SIGN_ELEMENT, CelestialBody } from "@astro-app/shared-types";
...
export function renderRadixToSvg(data, theme, width, height, padding = 20) {
    const cx = width / 2;
    const cy = height / 2;
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const glyphChar = SIGN_GLYPHS[sign];
        if (glyphChar) {
    ...
    for (const body of RENDERED_BODIES) {
        ...
        glyphPositions.push({ body: body, originalAngle: angle, displayAngle: angle, displaced: false });
    }
